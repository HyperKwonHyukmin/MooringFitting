...
{
  public class StructureCsvParserausing MooringFitting2026.RawData;
using MooringFitting2026.Utils.Geometry;
using System;
using System.Collections.Generic;
using System.IO;
...
namespace MooringFitting2026.Parsers
{
  public class StructureCsvParser
  {
    ...
    private MFData ParseMF(string[] parts)
    {
      ...
      // Tow 값이 존재하고 SWL보다 크면 SWL을 덮어씀
      double? tow = TryParseNullableDouble(parts[10]);
      if (tow.HasValue && tow.Value > swl)
      {
        swl = tow.Value;
      }

      // Rigid Range 파싱 (11~14번 인덱스)
      var rigidRange = new List<(double, double, double)>();
      for (int i = 11; i <= 14; i++)
      {
        string[] rangeParts = parts[i].Split('/');
      ...
      return new MFData(id, type, location, rigidRange) { /* SWL 등을 추가로 할당해야 한다면 여기에 */ };
      // *참고: 원본 코드에는 SWL 변수만 만들고 MFData 생성자에는 넣지 않았습니다.
      // 만약 MFData에 SWL 필드가 있다면 생성자에 추가해야 합니다.
    }
